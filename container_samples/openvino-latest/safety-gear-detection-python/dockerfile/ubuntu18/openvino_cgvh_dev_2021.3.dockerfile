...
ADD safety-gear-detection-python ${INTEL_OPENVINO_DIR}/python/samples/safety-gear-detection-python
RUN chown -R  intel:intel  ${INTEL_OPENVINO_DIR} ${INTEL_OPENVINO_DIR}/python  ${INTEL_OPENVINO_DIR}/python/samples  ${INTEL_OPENVINO_DIR}/python/samples/safety-gear-detection-python ${INTEL_OPENVINO_DIR}/deployment_tools ${INTEL_OPENVINO_DIR}/deployment_tools/model_optimizer ${INTEL_OPENVINO_DIR}/deployment_tools/model_optimizer/install_prerequisites  /var/lib/dpkg

RUN chmod 777 ${INTEL_OPENVINO_DIR}/deployment_tools/model_optimizer/mo.py
#RUN chmod 777 ${INTEL_OPENVINO_DIR}/python/samples/safety-gear-detection-python/run_sample.sh
RUN chmod -R 777 ${INTEL_OPENVINO_DIR}/python/samples/
RUN chmod 777 ${INTEL_OPENVINO_DIR}/python/samples/safety-gear-detection-python/*.sh
RUN apt-get install ffmpeg libsm6 libxext6 -y 
...
COPY safety-gear-detection-python/dockerfile/ubuntu18/config.txt ${INTEL_OPENVINO_DIR}/python/samples/safety-gear-detection-python/config.txt
RUN source  ${INTEL_OPENVINO_DIR}/bin/setupvars.sh 
RUN echo "Generating OpenVINO IR files ......."
RUN echo "Executing run_safety gear detection app using OpenVINO ......."
WORKDIR ${INTEL_OPENVINO_DIR}/python/samples/safety-gear-detection-python
# ENTRYPOINT  ${INTEL_OPENVINO_DIR}/python/samples/safety-gear-detection-python/run_safety_gear_detection.sh
ENTRYPOINT /bin/bash -c "source ${INTEL_OPENVINO_DIR}/python/samples/safety-gear-detection-python/run_safety_gear_detection.sh config.txt"
