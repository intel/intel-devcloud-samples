...
#RUN chmod 777 /opt/intel/openvino_2021.3.394/deployment_tools/model_optimizer/install_prerequisites/install_prerequisites_caffe.sh 
...
ADD tiny-yolo-v3-python ${INTEL_OPENVINO_DIR}/python/samples/tiny-yolo-v3-python
RUN chown -R  intel:intel  ${INTEL_OPENVINO_DIR} ${INTEL_OPENVINO_DIR}/python  ${INTEL_OPENVINO_DIR}/python/samples  ${INTEL_OPENVINO_DIR}/python/samples/tiny-yolo-v3-python ${INTEL_OPENVINO_DIR}/deployment_tools ${INTEL_OPENVINO_DIR}/deployment_tools/model_optimizer ${INTEL_OPENVINO_DIR}/deployment_tools/model_optimizer/install_prerequisites  /var/lib/dpkg

RUN chmod 777 ${INTEL_OPENVINO_DIR}/deployment_tools/model_optimizer/mo.py
#RUN chmod 777 ${INTEL_OPENVINO_DIR}/python/samples/tiny-yolo-v3-python/run_sample.sh
RUN chmod -R 777 ${INTEL_OPENVINO_DIR}/python/samples/
RUN chmod 777 ${INTEL_OPENVINO_DIR}/python/samples/tiny-yolo-v3-python/*.sh
RUN apt-get install ffmpeg libsm6 libxext6 -y 
...
COPY tiny-yolo-v3-python/dockerfile/ubuntu18/config.txt ${INTEL_OPENVINO_DIR}/python/samples/tiny-yolo-v3-python/config.txt
RUN source  ${INTEL_OPENVINO_DIR}/bin/setupvars.sh 
RUN echo "Generating OpenVINO IR files ......."
RUN echo "Executing run tiny-yolo-v3-python app using OpenVINO ......."
WORKDIR ${INTEL_OPENVINO_DIR}/python/samples/tiny-yolo-v3-python
RUN alias python3=python3.6
ENTRYPOINT /bin/bash -c "source ${INTEL_OPENVINO_DIR}/python/samples/tiny-yolo-v3-python/run_tiny_yolo_v3.sh config.txt"
#ENTRYPOINT ["/bin/bash", "/opt/intel/openvino/python/samples/tiny-yolo-v3-python/run_tiny_yolo_v3.sh"]
