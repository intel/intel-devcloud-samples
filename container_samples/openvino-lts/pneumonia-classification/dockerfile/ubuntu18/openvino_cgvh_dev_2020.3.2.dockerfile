...
RUN echo "Intel devcloud pnuemnia classification sample containerization begin ......."
...
ADD pneumonia-classification  ${INTEL_OPENVINO_DIR}/python/samples/pneumonia-classification
...
COPY pneumonia-classification/dockerfile/ubuntu18/config.txt ${INTEL_OPENVINO_DIR}/python/samples/pneumonia-classification/config.txt

RUN source  ${INTEL_OPENVINO_DIR}/bin/setupvars.sh
RUN echo "Generating OpenVINO IR files ......."
RUN echo "Executing pneumnia classification lts  app using OpenVINO ......."
WORKDIR ${INTEL_OPENVINO_DIR}/python/samples/pneumonia-classification
ENTRYPOINT /bin/bash -c "source ${INTEL_OPENVINO_DIR}/python/samples/pneumonia-classification/run_pneumonia.sh config.txt"
