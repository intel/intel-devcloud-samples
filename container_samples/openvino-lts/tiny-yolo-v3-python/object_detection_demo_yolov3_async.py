...
def main():
    ...
    #job_id = "skylake "#str(os.environ['PBS_JOBID']).split('.')[0]
    ...
    inference_file = output_path + "i_progress.txt"
    ...
    numbatchimgs,channels,input_height,input_width = net.inputs[input_blob].shape
    # Read and pre-process input images
    if channels == 3:
        nchw_shape = True
    else:
        nchw_shape = True #False
    ...
    #had to comment out due to FPGA not having proper bitstream that can support all layers in network topology
    ...
    presenter = monitors.Presenter(args.utilization_monitors, 55,
        (round(cap.get(width) / 4), round(cap.get(height) / 8)))
    start = time.time()
    while (cap.isOpened() \
        ...
        if next_frame_id_to_show in completed_request_results:
            ...
            #applicationMetricWriter.send_inference_time(duration*1000)
            #print("This is inference time", time.time() - start_time)
    ...
    for mode_value in mode_info.keys():
        ...
        with open(os.path.join(output_path, f'performance_results.txt'), 'w') as f:
