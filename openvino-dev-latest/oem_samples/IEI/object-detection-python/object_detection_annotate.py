...
def placeBoxes(frame, rd):
    ...
    color = (min(int(rd.class_id) * 7, 255), min(int(rd.class_id) * 35, 255), min(int(rd.class_id) * 5, 255))
    ...
    cv2.putText(frame, inf_time_message, (20, 25), cv2.FONT_HERSHEY_COMPLEX, 1, (200, 10, 10), 2)
    cv2.putText(frame, async_mode_message, (20, 50), cv2.FONT_HERSHEY_COMPLEX, 1,(10, 10, 200), 2)
...
def main():
    ...
    input_data = f"{args.output_dir}{job_id}/output.txt"
    progress_data = f"{args.output_dir}{job_id}/post_progress.txt"
    output_stream = f"{args.output_dir}{job_id}/output_{job_id}.mp4"
