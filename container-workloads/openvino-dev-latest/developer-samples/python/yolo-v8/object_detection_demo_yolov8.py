...
def build_argparser():
	...
	args.add_argument("-i", "--input", help="Required. Path to an image folder", required=True, type=str)
...
for img in os.listdir(IMAGE_PATH):
    input_image = np.array(Image.open(IMAGE_PATH + '/' + img))
    start_time = time.time()
    detections = detect(input_image, det_compiled_model)[0]
    perf.append((time.time() - start_time)*1000)
    image_with_boxes = draw_results(detections, input_image, label_map)
    Image.fromarray(image_with_boxes)
    cv2.imwrite(RESULTS_PATH + img, image_with_boxes)

latency = sum(perf)
avg_time = latency/len(perf)
throughput = 1000/avg_time

with open(RESULTS_PATH + 'performance.txt', 'w') as file:
    file.write("Throughput = " +str(throughput) + " FPS \n")
    file.write("Latency = " + str(latency) + " ms \n")
