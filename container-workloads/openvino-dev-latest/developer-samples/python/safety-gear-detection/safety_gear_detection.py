...
def process_boxes(frame_count, res, labels_map, prob_threshold, initial_w, initial_h, result_file, infer_time):
    for obj in res:
        dims = ""
...
def preprocess_video(video_input, pre_infer_file, processed_vid, batch_size,channels, height, width):
...
def main():
    ...
    # Compile the model for performance optimized for Throughput with  "PERFORMANCE_HINT", can be toggled for Latency 
 ...
 # Start Async Inference
    ...
    with open(os.path.join(args.output_dir, 'stats.json'), 'w', encoding="utf-8") as file:
