...
void printHelp() {
    ...
    std::cout << "Example: ./run_squeezenet --use_openvino <device_type> <path_to_the_model> <path_to_the_image> <path_to_the_classes_file>" << std::endl;
    std::cout << " where <device_type> options available for OpenVINO-EP are CPU_FP32, GPU_FP32, GPU_FP16, MYRIAD_FP16" << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    std::string outputPath = "/mount_folder";
    ...
    if (useOPENVINO)
    {
        ...
        logs << "Inference Execution Provider: OPENVINO\n";
    }
    else
    {
        std::cout << "Inference Execution Provider: CPU" << std::endl;
        logs<< "Inference Execution Provider: CPU\n";
    }

    std::string instanceName{"image-classification-inference"};

    std::string modelFilepath = argv[3]; // .onnx file
    ...
    std::string imageFilepath = argv[4];
    ...
    if (useOPENVINO) {
        ...
        options.device_type = devicetype.c_str(); //Other options are: GPU_FP32, GPU_FP16, MYRIAD_FP16
        std::cout << "OpenVINO device type is set to: " << options.device_type << std::endl;
        logs << "OpenVINO device type is set to: " << options.device_type << "\n";
    ...
    logs << "Predicted Label ID: " << predId << std::endl;
    logs << "Predicted Label: " << labels.at(predId) << std::endl; 
    float result;
    try {
    ...
    std::cout << "Minimum Inference Latency: "
              ...
              << " ms" << std::endl;
    logs << "Minimum Inference Latency: "
         << std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count() / static_cast<float>(numTests))
         << " ms" << "\n";
    logs << "Total time taken for 100 tests: "
